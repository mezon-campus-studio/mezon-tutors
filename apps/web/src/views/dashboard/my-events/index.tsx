...
import { useMyEventSubmissions } from "@/services";
...
export default function MyEventsView() {
  ...
  const { data: events = [], isLoading } = useMyEventSubmissions();
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link href={ROUTES.EVENTS.CREATE} className="shrink-0">
          <Button
            variant="gradient"
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-violet-200 bg-[linear-gradient(180deg,#faf7ff_0%,#ffffff_100%)] px-6 py-16 text-center">
          ...
          <Link href={ROUTES.EVENTS.CREATE} className="mt-6">
            <Button
              variant="gradient"
        ...
        <div className="space-y-4">
          {events.map((event) => {
            ...
            const statusStyle = STATUS_STYLES[event.publishStatus] ?? STATUS_STYLES.PENDING;
            const canEdit =
              event.publishStatus === "PENDING" ||
              event.publishStatus === "REJECTED" ||
              ...
              >
                <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:p-5">
                  ...
                  <div className="min-w-0 flex-1 space-y-2">
                    ...
                    <p className="line-clamp-2 text-sm text-slate-600">{content.tagline}</p>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                      ...
                      <span>
                        {t("submittedAt", {
                          date: dayjs(event.createdAt).format("DD/MM/YYYY"),
                        })}
                      </span>
                    </div>

                    {event.publishStatus === "REJECTED" && event.rejectedReason ? (
