...
export default function ProfileTabs({ activeTab, onTabChange, className }: ProfileTabsProps) {
  ...
  return (
    <div
      className={cn(
        'flex gap-1 overflow-x-auto rounded-2xl border border-violet-100 bg-white/80 p-1.5 shadow-sm shadow-violet-100/40 backdrop-blur',
        className,
      )}
      role="tablist"
    >
      {TABS.map(({ number: tabNumber, id }) => {
        ...
        return (
          <button
            ...
            className={cn(
              'min-w-0 flex-1 cursor-pointer whitespace-nowrap rounded-xl px-3 py-2.5 text-xs font-semibold transition-all sm:px-4 sm:text-sm',
              isActive
