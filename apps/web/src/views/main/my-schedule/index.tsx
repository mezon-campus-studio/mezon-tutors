...
export default function MyScheduleView() {
  ...
  const [pickedEvent, setPickedEvent] = useState<ScheduleEventItem | null>(
    null,
  );
  ...
  const eventsThisWeek = useMemo(() => {
    return items
      .map((item) =>
        toScheduleEvent(item, weekStart, weekEnd, locale, userTimezone),
      )
  ...
  const isCurrentWeek = weekStart.isSame(
    buildWeekStartMonday(dayjs().tz(userTimezone)),
    "day",
  );
  ...
  const scheduleEventDetailRows = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: pickedEvent.isCompleted
          ? t("completedBadge")
          : tModal("statusUpcoming"),
      },
    ];
