...
export default function AvailabilityPage() {
  ...
  const handleContinue = async () => {
    ...
    const price = payload.pricePerHour;

    const fallbackRates = { usd: 1, vnd: 25000, php: 57 };
    
    let baseUsd = price;
    if (currency !== ECurrency.USD) {
      baseUsd = currentRates?.usd ? (price * currentRates.usd) : (price / fallbackRates[currency.toLowerCase() as keyof typeof fallbackRates]);
    }

    const usd = baseUsd;
    const vnd = currency === ECurrency.VND ? price : (currentRates?.vnd ? (price * currentRates.vnd) : (baseUsd * fallbackRates.vnd));
    const php = currency === ECurrency.PHP ? price : (currentRates?.php ? (price * currentRates.php) : (baseUsd * fallbackRates.php));

    payload.prices = { usd, vnd: Math.round(vnd), php };

    await submitMutation.mutateAsync(payload);
    queryClient.setQueryData(tutorProfileQueryKey.myTutorProfileStatus(), {
      hasProfile: true,
